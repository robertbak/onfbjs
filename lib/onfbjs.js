        ...
        if (options.authentication_redirect_url) {
            ...
            if (req.session.signed_request != null && req.session.signed_request.page != null) {
                ...
                if ( linkDictionary['id_'+req.session.signed_request.page.id] != null){
                    res.send(fbtools.generateAskForPermissionsPage(options.app_id, fbtools.generateRedirectUrl(linkDictionary['id_'+req.session.signed_request.page.id], options.app_id), options.permissions));
                    return;
                }
